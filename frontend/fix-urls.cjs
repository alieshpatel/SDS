...
files.forEach(file => {
    ...
    // Also user said: axios.get("http://localhost:5000/api/xyz") or fetch("http://localhost:5000/api/xyz")
    content = content.replace(/fetch\([`'"]http:\/\/localhost:5000([^`'"]+)[`'"]\)/g, "fetch(`$1`)"); // For fetch, although we're using api now.
